...
assert.deepEqual(batches, []);
relatedChoice = true;
assert.deepEqual((await confirmRelated.call(relatedEditor, { id: '1' })).map(c => c.id), ['0', '1', '2']);
assert.deepEqual(batches, [], 'confirming the chain must not queue individual workflows');
