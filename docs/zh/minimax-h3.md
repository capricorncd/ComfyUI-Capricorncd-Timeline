...
Clip 类型为「数字人」时，Cap MiniMaxH3 将时间轴驱动音频编码到输出 latent，并设置音频噪声遮罩为零。直接连接采样器时保留完整 LATENT；若拆分、拼接或二次采样，请保留音频及其 noise_mask。输出 audio 是补齐 Context 前缀静音和模型对齐长度后的源音轨，可用于保存视频；裁掉 Context 时音视频需一起裁剪。此处理与 MiniMax H3 Video Generator 共用，不会重复编码。
