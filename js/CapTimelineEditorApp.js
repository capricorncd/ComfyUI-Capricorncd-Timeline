...
export class CapTimelineEditorApp {
    ...
    handleGenEditKey(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && !e.altKey && !e.shiftKey && ["c", "v", "x", "b"].includes(key)) {
            ...
            if (!e.repeat) {
                if (key === "c") this._copyGenEditClips();
                else if (key === "v") this._pasteGenEditClips();
                else {
                    const selected = this._genEditState?.timeline?.getSelectedClips() || [];
                    if (selected.length === 1) {
                        if (key === "x") this._splitGenEditClip(selected[0]);
                        else this._toggleGenEditClipState(selected[0], selected[0].track.type === "audio" ? "muted" : "enabled");
                    }
                }
            }
            return true;
    ...
    _showRunMenu(e) {
        ...
        return this._buildCtxMenu([
            ...
            { separator: true },
            { label: draftT("generate_all"), icon: "listCollapse", fn: () => void this._runAllActiveClipsDownstream({ h3Generation: {action: "draft"} }) },
            { label: draftT("generate_selected"), icon: "listCollapse", fn: () => void this._runSelectedClipsDownstream({action: "draft"}) },
        ], r.left, r.bottom + 4, { ignoreNextClick: false });
    }
    ...
    async _runSelectedClipsDownstream(h3Generation = null) {
    ...
    async _runAllActiveClipsDownstream({ withoutGenerated = false, clips: requestedClips = null, h3Generation = null } = {}) {
        let clips = Array.isArray(requestedClips)
            ? requestedClips
        ...
        if (this._hasH3VideoGeneratorDownstream()) {
            try {
                await this._queueClipsDownstream(clips, null, h3Generation);
            } finally {
            ...
            try {
                ...
                if (job?.h3Generation && editor) {
                    for (const node of Object.values(result.output || {})) {
                        ...
                        const expectedIds = job.clipIds || [job.clipId];
                        if (ids?.length !== expectedIds.length || ids.some((id, i) => String(id) !== String(expectedIds[i]))) continue;
                        project.settings.h3_generation = job.h3Generation;
    ...
    _ensureOverlay() {
              ...
              <div class="cat-te-clip-info">
                <div class="cat-te-clip-info-body">
                  <div class="cat-te-clip-info-detail" hidden>
                    <cap-media-carousel class="cat-te-clip-swiper" previous-label="${T("prev_material")}" next-label="${T("next_material")}">
              ...
              <div class="cat-te-ai-optimize-body">
                <div class="cat-te-ai-optimize-left">
                  ...
                  <div class="cat-te-ai-resource-pane" hidden>
                    <cap-media-carousel class="cat-te-ai-resource-preview" previous-label="${T("prev_material")}" next-label="${T("next_material")}">
                      <div class="cat-te-ai-resource-stage"></div>
                    </cap-media-carousel>
                ...
                <div class="cat-te-ai-optimize-right">
                  ...
                  <div class="cat-te-ai-context">
                    <div class="cat-te-ai-context-title">${T("ai_context_title")}</div>
                  ...
                  <div class="cat-te-ai-field-row">
                    <label class="cat-te-ai-field">
                      <span>${T("prompt_target_agent_label")}</span>
                      <select class="cat-te-ai-target-agent">
                        <option value="MiniMaxH3">MiniMaxH3</option>
                        <option value="LTX">LTX</option>
                        <option value="Bernini">Bernini</option>
                        <option value="Wan">Wan</option>
                    ...
                    <label class="cat-te-ai-field">
                      <span>${T("audio_usage_label")}</span>
                      <select class="cat-te-ai-audio-mode">
                        <option value="auto">${T("audio_usage_auto")}</option>
                        <option value="perform">${T("audio_usage_perform")}</option>
                        <option value="lipsync">${T("audio_usage_lipsync")}</option>
                        <option value="none">${T("audio_usage_none")}</option>
                      </select>
                    </label>
                  </div>
            ...
            <div class="cat-te-modal cat-te-settings-dialog">
              ...
              <div class="cat-te-modal-body cat-te-settings-layout">
                <nav class="cat-te-settings-nav" aria-label="${T("settings_title")}">
                  ...
                  <cap-button data-settings-category="bgm" aria-pressed="false" align="start">${T("local_audio_settings")} <span class="cat-te-info-tip" tabindex="0" aria-label="${T("audio_override_note")}">${iconHtml("info", 12)}<span class="cat-te-info-tip-pop">${T("audio_override_note")}</span></span></cap-button>
                </nav>
        ...
        this.aiTargetAgentSelect = el.querySelector(".cat-te-ai-target-agent");
        this.aiTargetRoleSelect = el.querySelector(".cat-te-ai-target-role");
        ...
        this.aiSourceEditor = el.querySelector(".cat-te-ai-source-editor");
        this.aiResourcePane = el.querySelector(".cat-te-ai-resource-pane");
        ...
        this.aiTargetAgentSelect?.addEventListener("change", () => void this._onAiPromptTargetChange());
        this.aiTargetRoleSelect?.addEventListener("change", () => void this._onAiPromptTargetChange());
        this.aiContextInputs?.forEach((input) => {
            input.addEventListener("change", () => {
    ...
    _setupTrackControls(track) {
        ...
        if (icon && !icon.dataset.catTeTypeMenuBound) {
            ...
            track.headerEl.addEventListener('contextmenu', event => {
                event.preventDefault();
                event.stopPropagation();
                this._showTrackTypeMenu(track, icon);
            });
        }
    ...
    _onTimelineVideoSaved(e) {
        ...
        if (!file || file.startsWith("capricorncd-timeline/h3_drafts/")) return;
    ...
    _collectExecutedOutputVideos(detail) {
        ...
        const add = (file) => {
            const n = normalizeOutputVideoPath(file);
            if (!n || n.startsWith("capricorncd-timeline/h3_drafts/") || seen.has(n)) return;
            seen.add(n);
            out.push(n);
        };
    ...
    _buildGenEditTimeline() {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (c) {
                ...
                void this._ensureGenVideoAudioBuffer(gen.file).then(buffer => {
                    ...
                    c.waveformPeaks = this._audioBufferToPeaks(buffer)[0];
                });
            }
        }
        ...
        for (const row of audioRows) {
            ...
            if (c) {
                ...
                c.el.classList.toggle("cat-te-clip-muted", row.muted === true);
                this._syncGenEditClipDisabled(c, row.enabled !== false);
                c.hasAudio = true;
                c.waveformVolume = normalizeClipVolume(row.volume);
        ...
        tl.scrollEl?.addEventListener("contextmenu", (e) => {
            ...
            if (!clipEl) {
                e.preventDefault();
                e.stopPropagation();
                this._buildCtxMenu([{ label: T("menu_paste_shortcut"), icon: "clipboard", disabled: !st.clipClipboard?.length,
                    fn: () => this._pasteGenEditClips() }], e.clientX, e.clientY);
                return;
            }
            e.preventDefault();
            e.stopPropagation();
            ...
            const row = isAudioClip ? st.audioDraft.find(a => a.id === st.audioMap.get(c.id)) : st.draft.find(g => g.id === gid);
            if (row) items.unshift(
                { label: T(row.enabled === false ? "enabled_label" : "disable_label"), icon: row.enabled === false ? "eye" : "eyeOff", disabled: c.track.locked,
                    fn: () => this._toggleGenEditClipState(c, "enabled") },
                { label: T(row.muted ? "unmute_label" : "mute_label"), icon: row.muted ? "volume" : "volumeOff", disabled: c.track.locked,
                    fn: () => this._toggleGenEditClipState(c, "muted") },
            );
    ...
    _setupGenEditTrackControls(track, genId) {
        const icon = track.headerEl?.querySelector(".tl-track-icon");
        if (icon) icon.innerHTML = ICONS.film;
        const actions = track.actionsEl;
        if (!actions) return;
        actions.replaceChildren();
    ...
    _copyGenEditClips() {
        ...
        this._pullGenEditDraftFromTimeline();
        const selected = st.timeline.getSelectedClips();
        const ids = new Set(selected.map(clip => st.audioMap.get(clip.id)));
        const videoIds = new Set(selected.map(clip => st.clipMap.get(clip.id)));
        const rows = [
            ...st.audioDraft.filter(row => ids.has(row.id)),
            ...st.draft.filter(row => videoIds.has(row.id)).map(row => ({ ...row, clipboardType: "video" })),
        ];
        if (!rows.length) return false;
        st.clipClipboard = structuredClone(rows);
        return true;
    }

    _pasteGenEditClips() {
        const st = this._genEditState;
        if (!st?.timeline || !st.clipClipboard?.length) return false;
        this._pullGenEditDraftFromTimeline();
        const time = st.timeline.currentTime;
        const start = Math.min(...st.clipClipboard.map(row => row.edit_start_sec));
        const rows = structuredClone(st.clipClipboard).map(row => ({ ...row, id: row.clipboardType === "video" ? genVideoUid() : genAudioUid(),
            from_gen_id: null, edit_start_sec: time + row.edit_start_sec - start }));
        for (const row of rows) {
            if (row.clipboardType === "video") {
                delete row.clipboardType;
                delete row.from_gen_id;
                st.draft.push(row);
                continue;
            }
        ...
        for (const track of st.timeline.tracks) {
            for (const clip of track.clips) {
                if (ids.has(st.audioMap.get(clip.id)) || ids.has(st.clipMap.get(clip.id))) st.timeline.selectClip(clip, { additive: true });
            }
        }
        this._syncGenEditInspector();
        this._scheduleGenEditPreview();
    ...
    _toggleGenEditClipState(clip, field) {
        const st = this._genEditState;
        if (!st || !clip || clip.track?.locked) return;
        const time = st.timeline.currentTime;
        this._pullGenEditDraftFromTimeline();
        const row = st.audioDraft.find(a => a.id === st.audioMap.get(clip.id))
            || st.draft.find(g => g.id === st.clipMap.get(clip.id));
        if (!row) return;
        row[field] = field === "enabled" ? row.enabled === false : !row.muted;
        this._applyGenEditChanges();
        this._buildGenEditTimeline();
        st.timeline.setCurrentTime(time);
        for (const track of st.timeline.tracks) {
            const selected = track.clips.find(c => st.audioMap.get(c.id) === row.id || st.clipMap.get(c.id) === row.id);
            if (selected) st.timeline.selectClip(selected);
        }
        this._syncGenEditInspector();
        this._scheduleGenEditPreview();
    }
    ...
    _splitGenEditClip(clip) {
        const st = this._genEditState;
        const tl = st?.timeline;
        if (!st || !tl || !clip || clip.track?.locked) return;
        const t = tl.currentTime;
        if (!(t > clip.startTime + 1e-3 && t < clip.endTime - 1e-3)) return;
        this._pullGenEditDraftFromTimeline();
        const aid = st.audioMap.get(clip.id);
        if (aid) {
            const index = st.audioDraft.findIndex(row => row.id === aid);
            if (index < 0) return;
            const source = st.audioDraft[index];
            const local = t - clip.startTime;
            const right = { ...source, id: genAudioUid(), edit_start_sec: t,
                source_offset: source.source_offset + local, duration: source.duration - local,
                volume_points: normalizeVolumePoints(source.volume_points) };
            source.duration = local;
            st.audioDraft.splice(index + 1, 0, right);
            this._applyGenEditChanges();
            this._buildGenEditTimeline();
            st.timeline.setCurrentTime(t);
            this._syncGenEditInspector();
            this._scheduleGenEditPreview();
            return;
        }
    ...
    _clipDenoiseSources(clip, includeGenerated = false) {
        if (!clip || (isDirectorTrackType(clip.track?.type) && !includeGenerated)) return [];
        const meta = this._ensureClipMeta(clip);
        if (includeGenerated && isDirectorTrackType(clip.track?.type)) {
            const mix = this._collectGeneratedVideoAudioJobs(clip.startTime, clip).map(job => ({
        ...
        if (includeGenerated && this._clipUsesGeneratedPreview(meta)) {
            return [
                ...
                ...this._normalizeGenEditAudioDraft(meta.genEditAudios).filter(row => row.enabled !== false).map(row => ({
                    detachedId: row.id, file: row.file, location: 'input', trim_in_sec: row.source_offset,
                    duration_sec: row.duration, playback_rate: 1, edit_start_sec: row.edit_start_sec,
                })),
            ];
        }
        if (isVoiceoverTrackType(clip.track?.type)) {
            return this._clipGeneratedAudios(meta).filter(row => row.enabled !== false).map(row => ({
                id: row.id, file: row.file, location: "output", trim_in_sec: row.trim_in_sec || 0,
                duration_sec: Math.max(0.05, Math.min(clip.duration, (row.trim_out_sec ?? row.duration_sec ?? clip.duration) - (row.trim_in_sec || 0))),
                playback_rate: 1,
            }));
        }
        if (!clip.src || (clip.track?.type !== "audio" && !clip.hasAudio)) return [];
        return [{ file: clip.src, location: "input", trim_in_sec: clip.sourceOffset || 0,
            duration_sec: clip.duration, playback_rate: clip.playbackRate || 1 }];
    }

    _openGenEditDenoise(timelineClip, kind = 'denoise') {
        const state = this._genEditState;
        if (!state) return;
        const videoId = state.clipMap.get(timelineClip.id);
        const audioId = state.audioMap.get(timelineClip.id);
        const source = videoId ? state.draft.find(row => row.id === videoId) : state.audioDraft.find(row => row.id === audioId);
    ...
    async _insertDenoisedAudio(clip, context, result, valid) {
        if (!valid()) return false;
        if (context.state) {
            const state = context.state;
            if (this._genEditState !== state) return false;
            const source = context.videoId ? state.draft.find(row => row.id === context.videoId)
                : state.audioDraft.find(row => row.id === context.audioId);
            if (!source) return false;
            state.audioDraft.push({ id: genAudioUid(), file: result.file, edit_start_sec: context.start,
                duration: result.duration_sec, source_offset: 0, source_duration: result.duration_sec,
                muted: false, from_gen_id: context.videoId || null });
            source.muted = true;
            if (context.disableSource) source.enabled = false;
            this._applyGenEditChanges();
            this._buildGenEditTimeline();
            this._syncGenEditInspector();
            this._scheduleGenEditPreview();
        } else {
            await this._addAudioAtTime(result.file, context.start, null, { canInsert: valid, duration: result.duration_sec });
            if (!valid()) return false;
            const meta = this._ensureClipMeta(clip);
            if (context.source.id) {
                const row = meta.generatedAudios?.find(row => row.id === context.source.id);
                if (row) row.muted = true;
                if (this._voEditState?.clipId === clip.id) {
                    this._pullVoiceoverEditDraftFromTimeline();
                    const draft = this._voEditState.draft.find(row => row.id === context.source.id);
                    if (draft) draft.muted = true;
                    void this._buildVoiceoverEditTimeline();
                }
            } else if (context.source.videoId) {
                const row = meta.generatedVideos?.find(row => row.id === context.source.videoId);
                if (row) row.muted = true;
            } else if (context.source.detachedId) {
                const row = meta.genEditAudios?.find(row => row.id === context.source.detachedId);
                if (row) row.muted = true;
            } else meta.muted = true;
            this._decorateClip(clip);
            this._saveToWidgets();
            this._scheduleProgramPreview();
        }
    ...
    _onGenEditVolumeInput() {
        const st = this._genEditState;
        const selected = st?.timeline?.getSelectedClips()[0];
        if (!selected || selected.track.locked) return;
        this._pullGenEditDraftFromTimeline();
        const audioId = st.audioMap.get(selected.id);
        const row = audioId ? st.audioDraft.find(row => row.id === audioId)
            : st.draft.find(row => row.id === st.clipMap.get(selected.id));
        if (!row) return;
        row.volume = normalizeClipVolume(Number(this.genEditVolume.value) / 100);
        this.genEditVolumeValue.textContent = `${Math.round(row.volume * 100)}%`;
        selected.waveformVolume = row.volume;
        selected._paintWaveform();
        this._applyGenEditChanges();
        if (st.timeline._playing) void this._startGenEditAudioPlayback();
    }
    ...
    async _startGenEditAudioPlayback() {
        ...
        for (const job of jobs) {
            ...
            gain.gain.setValueAtTime(
                normalizeClipVolume(parentClip ? this._ensureClipMeta(parentClip)?.volume : 1) * normalizeClipVolume(job.volume),
                startCtxTime,
            );
    ...
    async _buildVoiceoverEditTimeline() {
        ...
        tl.scrollEl?.addEventListener("contextmenu", event => {
            const element = event.target.closest(".tl-clip");
            const item = tl.tracks.flatMap(track => track.clips).find(item => item.el === element);
            const row = st.draft.find(row => row.id === st.clipMap.get(item?.id));
            ...
            }, kind) })), event.clientX, event.clientY);
        });
    ...
    _syncClipPrimaryAppearance(clip, { refreshVideo = true } = {}) {
        clip.waveformWindow = null;
        clip.waveformAbsoluteScale = false;
        ...
        if (gen) {
            void this._syncGeneratedClipWaveform(clip);
            if (refreshVideo) {
    ...
    async _syncGeneratedClipWaveform(clip) {
        const token = {};
        clip._waveformRequest = token;
        const jobs = this._collectGeneratedVideoAudioJobs(clip.startTime, clip);
        const peaks = new Float32Array(8000);
        let hasAudio = false;
        for (const job of jobs) {
            const buffer = await this._ensureGenVideoAudioBuffer(job.file, job.location);
            if (clip._waveformRequest !== token || !this._clipUsesGeneratedPreview(this._ensureClipMeta(clip))) return;
            if (!buffer) continue;
            hasAudio = true;
            const source = this._audioBufferToPeaks(buffer)[0];
        ...
        if (clip._waveformRequest !== token) return;
        clip.hasAudio = hasAudio;
        clip.waveformWindow = { sourceOffset: 0, sourceDuration: clip.duration, playbackRate: 1 };
        clip.waveformVolume = 1;
        clip.waveformAbsoluteScale = true;
        clip.waveformPeaks = peaks;
        clip._refreshWaveRow();
    }
    ...
    _applyClipItemOrder(clip, orderedItems) {
        if (!clip || clip.track?.locked || !Array.isArray(orderedItems)) return false;
        const m = this._ensureClipMeta(clip);
    ...
    _removeClipItemNow(clip, itemId, fallbackIndex) {
        ...
        this._normalizeVisualMeta(clip, m, { seedFromClip: false });
        this._meta.set(clip.id, m);
        const nextSelected = selected > removeIndex ? selected - 1 : selected;
        this._setClipPreviewItemIndex(clip, Math.min(nextSelected, Math.max(0, m.items.length - 1)));
        this._syncClipPrimaryAppearance(clip);
        this._scheduleProgramPreview();
        this._saveToWidgets();
        this._refreshClipResourceViews(clip);
...
`;
    ...
    _bindMediaPointerDrag(item, kind, file) {
        ...
        item.addEventListener("pointerdown", (e) => {
            ...
            const onUp = (ev) => {
                ...
                this._dndMedia = null;
                if (ev.type === "pointercancel" || !started || !media?.file) return;
    ...
    _clipResourceDropTarget(clientX, clientY) {
        const carousel = document.elementFromPoint(clientX, clientY)?.closest("cap-media-carousel");
        if (!carousel) return null;
        if (carousel === this.clipSwiper) return { carousel, clip: this._selClip };
        if (carousel === this.aiResourceCarousel) return { carousel, clip: this._findClipById(this._aiOptimizeClipId) };
        return null;
    }

    _canDropClipResource(target, media) {
        return !!target?.clip && target.carousel.editable && !target.clip.track?.locked
            && (media?.kind === "image" || media?.kind === "video");
    }

    _updateMediaDragHover(clientX, clientY) {
        const target = this._clipResourceDropTarget(clientX, clientY);
        const resourceOver = this._canDropClipResource(target, this._dndMedia);
        for (const carousel of [this.clipSwiper, this.aiResourceCarousel]) {
            carousel?.toggleAttribute("drop-active", resourceOver && carousel === target.carousel);
        }
        const host = this.tlHost;
        const scroll = this._timeline?.scrollEl;
        if (!host && !scroll) return;
        const r = (host || scroll).getBoundingClientRect();
        const over = !target && clientX >= r.left && clientX <= r.right
            && clientY >= r.top && clientY <= r.bottom;
        host?.classList.toggle("cat-te-file-drop-over", over);
        scroll?.classList.toggle("cat-te-drop-active", over);
        document.body.classList.toggle("cat-te-media-dnd-over-tl", over || resourceOver);
    ...
    _clearMediaDragVisuals(item, ghost) {
        ...
        this.clipSwiper?.removeAttribute("drop-active");
        this.aiResourceCarousel?.removeAttribute("drop-active");
    ...
    _commitMediaDrop(media, clientX, clientY) {
        ...
        const resourceTarget = this._clipResourceDropTarget(clientX, clientY);
        if (resourceTarget) {
            this._dndHoverClip = null;
            if (this._canDropClipResource(resourceTarget, media)) {
                this._insertItemIntoClip(resourceTarget.clip, media.file, media.kind);
            }
            return;
        }
    ...
    async _scheduleGeneratedVideoWebAudio(startCtxTime, startPlayhead, { jobs = null, isPlaying = () => !!this._timeline?._playing } = {}) {
        ...
        for (const job of jobs || this._collectGeneratedVideoAudioJobs(startPlayhead)) {
            ...
            src.connect(gain);
            gain.connect(ctx.destination);
            gain.gain.setValueAtTime(job.volume, startCtxTime);
    ...
    _scheduleAudioFadeGain(gainNode, when, localStart, playDur, fadeIn, fadeOut, clipDur, volume = 1, volumePoints = [], sourceOffset = 0, rate = 1) {
        ...
        const gAt = (t) => {
            ...
            return clamp01(g) * volume * volumeAt(envelope, (sourceOffset + t * rate) * 1000);
        };
    ...
    _receiveH3Draft(version) {
        ...
        if (clip) {
            const meta = this._ensureClipMeta(clip);
            if (meta.h3DraftRemoved?.includes(version.id)) return;
            meta.h3Drafts ||= [];
    ...
    _splitClip(clip) {
        ...
        // Keep the track populated while splitting so empty-track cleanup cannot remove it.
    ...
    _clearClipInfoPanel() {
        ...
        this.clipSwiper?.setItems([], { allowList: false });
    ...
    _onClipVolumeInput(commit) {
        ...
        if (this._clipUsesGeneratedPreview(m)) void this._syncGeneratedClipWaveform(this._selClip);
        else {
            this._selClip.waveformVolume = m.volume;
            this._selClip._paintWaveform();
        }
    ...
    _clipAiOptimizeFiles(clip, context = DEFAULT_AI_PROMPT_CONTEXT) {
        ...
        for (const track of this._timeline?.tracks || []) {
            ...
            if (info.enabled === false) continue;
            for (const audioClip of track.clips || []) {
                const audioStart = Number(audioClip.startTime) || 0;
                const audioEnd = audioStart + Math.max(0, Number(audioClip.duration) || 0);
                if (audioEnd <= start || audioStart >= end) continue;
                const audioMeta = this._meta.get(audioClip.id) || defaultAudioMeta(this._trackIndex(track));
                if (audioMeta.muted) continue;
    ...
    _refreshClipResourceViews(clip) {
        if (this._selClip?.id === clip.id) this._updateClipInfoPanel(clip);
        if (this._aiOptimizeClipId === clip.id && this._aiOptimizeSrc === "resource") this._renderAiResource();
    }

    _bindClipResourceCarousel(carousel, getClip) {
        carousel.addEventListener("media-change", event => {
            const clip = getClip();
            if (!clip) return;
            this._setClipPreviewItemIndex(clip, event.detail.index);
            this._refreshClipResourceViews(clip);
        });
        ...
        carousel.addEventListener("media-edit", event => {
            ...
            if (action === "reorder" && Number.isInteger(from) && Number.isInteger(to)
                && from >= 0 && from < items.length && to >= 0 && to < items.length && from !== to) {
                const selected = this._clipPreviewItemIndex(clip, this._ensureClipMeta(clip));
                const order = items.map((_, i) => i);
                order.splice(to, 0, order.splice(from, 1)[0]);
                if (this._applyClipItemOrder(clip, order.map(i => items[i]))) {
                    this._setClipPreviewItemIndex(clip, order.indexOf(selected));
                    this._refreshClipResourceViews(clip);
                }
            }
        });
    ...
    _clearAiResourcePreview() {
        for (const media of this.aiResourceStage?.querySelectorAll("video, audio") || []) {
            media.pause();
            media.removeAttribute("src");
            media.load();
        }
    ...
    _renderAiResource() {
        ...
        const media = (item.id && this._findMediaById(item.id)) || this._findMedia(item.kind, item.file);
        if (this.aiResourceCarousel.mode === "preview") {
            const location = this._mediaStatus.get(`${item.kind}:${item.file}`)?.location || media?.location || "input";
            const preview = document.createElement(item.kind === "video" ? "video" : item.kind === "audio" ? "audio" : "img");
            if (item.kind === "video" || item.kind === "audio") {
                preview.controls = true;
                preview.preload = "metadata";
                if (item.kind === "video") preview.playsInline = true;
            } else {
                preview.alt = String(media?.name || item.file);
            }
            preview.src = this._assetFileUrl(item.file, item.kind, location);
            this.aiResourceStage.appendChild(preview);
        }
        this.aiResourceName.textContent = String(media?.name || item.file);
        this.aiResourceDescription.value = String(media?.setting_description || "");
        this.aiResourceDescription.placeholder = T("prompt_resource_no_description");
        this.aiResourceDescription.scrollTop = 0;
    }
    ...
    _setAiOptimizeSrcTab(tab = "clip") {
        ...
        if (this.aiSourceEditor) this.aiSourceEditor.hidden = next === "resource";
        if (this.aiResourcePane) this.aiResourcePane.hidden = next !== "resource";
        if (next !== "resource") this._clearAiResourcePreview();
        this._promptManagerUndoArmed = false;
    ...
    _fillAiOptimizeSrc() {
        if (this._aiOptimizeSrc === "resource") {
            this._renderAiResource();
            return;
        }
    ...
    _aiPromptContext() {
        const out = { ...DEFAULT_AI_PROMPT_CONTEXT };
        try {
            const saved = JSON.parse(localStorage.getItem(STORAGE_AI_PROMPT_CONTEXT) || "{}");
        ...
        this.aiContextInputs?.forEach((input) => {
            if (input.dataset.context in out) out[input.dataset.context] = input.checked;
        });
        return out;
    }

    _restoreAiPromptContext() {
        let saved = {};
        try { saved = JSON.parse(localStorage.getItem(STORAGE_AI_PROMPT_CONTEXT) || "{}"); } catch { /* use defaults */ }
        this.aiContextInputs?.forEach((input) => {
            const key = input.dataset.context;
            input.checked = typeof saved?.[key] === "boolean" ? saved[key] : DEFAULT_AI_PROMPT_CONTEXT[key] !== false;
        });
        this._syncAiPromptContextControls();
    }

    _saveAiPromptContext() {
        localStorage.setItem(STORAGE_AI_PROMPT_CONTEXT, JSON.stringify(this._aiPromptContext()));
    }

    _syncAiPromptContextControls() {
        const context = this._aiPromptContext();
        if (this.aiAudioModeSelect) this.aiAudioModeSelect.disabled = !context.audio_data || this._aiOptimizeBusy;
    }

    _aiPromptTargetAgent() {
        return String(this.aiTargetAgentSelect?.value || "MiniMaxH3");
    }

    async _onAiPromptTargetChange() {
        const clip = this._findClipById(this._aiOptimizeClipId) || this._selClip;
        if (!clip) return;
        this._recordUndo();
        const meta = this._ensureClipMeta(clip);
        ...
        this._meta.set(clip.id, meta);
        this._saveToWidgets();
        await this._loadAiAgentPrompt(meta.agent, meta.clipRole);
    ...
    async _bindAiOptimizeToClip(clip, { reloadModels = false } = {}) {
        ...
        const targetAgent = ["MiniMaxH3", "LTX", "Bernini", "Wan", "other"].includes(meta.agent)
            ? meta.agent
            : "MiniMaxH3";
        if (this.aiTargetAgentSelect) this.aiTargetAgentSelect.value = targetAgent;
        if (this.aiTargetRoleSelect) this.aiTargetRoleSelect.value = this._knownClipRole(meta.clipRole);
        ...
        await this._loadAiAgentPrompt(targetAgent, meta.clipRole || "multi_ref");
        this._syncAiPromptTargetControls();
        this._syncAiOptimizeNavButtons();
    ...
    _setAiOptimizeBusy(busy) {
        ...
        if (this.aiTargetAgentSelect) this.aiTargetAgentSelect.disabled = !!busy;
        if (this.aiTargetRoleSelect) this.aiTargetRoleSelect.disabled = !!busy;
        this.aiContextInputs?.forEach((input) => { input.disabled = !!busy; });
        if (busy) {
    ...
    async _runAiOptimize() {
        ...
        try {
            const payload = {
                ...
                agent: targetAgent,
                clip_role: String(this.aiTargetRoleSelect?.value || meta.clipRole || "multi_ref"),
    ...
    _setSkillSyncBusy(busy) {
        ...
        this.skillSyncBtn.disabled = !!busy || this._aiPromptTargetAgent() !== "MiniMaxH3";
                ...
                if (track.type === "audio") {
                    ...
                    if (genAudios.length) {
                        row.gen_edit_audios = genAudios.map((a) => ({
                            ...
                            volume: normalizeClipVolume(a.volume),
                            volume_points: normalizeVolumePoints(a.volume_points),
