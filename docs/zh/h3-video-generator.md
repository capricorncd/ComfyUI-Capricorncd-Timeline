...
- 连续片段一起运行时，按 `previous_source_clip_id` 匹配本次运行的低清/高清 Context，保存在 `output/h3_context/cap_generator/<本次运行>/`。单独运行续接片段时，使用前段最新且已启用的关联生成视频，截取指定 Context 帧数的尾部画面及对应音频；二采在目标分辨率重新编码这段参考。前段 Context 和视频均不可用时，改为独立生成并重新计算裁剪；节点显示警告，完成后仍保留。不读取目录中任意“最新文件”。
