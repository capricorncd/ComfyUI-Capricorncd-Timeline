...
禁用版本仍可预览，不能作为精修选择。删除只移除工程关联，保留磁盘文件；撤销可恢复关联。版本文件位于 `output/capricorncd-timeline/h3_drafts/`，当前工程包不打包这些 latent，跨机器使用需另外保留该目录和参考素材。自动二采沿用生成器的放大模型和二采参数。升级后重启 ComfyUI 并刷新浏览器以更新节点参数。
