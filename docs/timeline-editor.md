...
- Audio-track clips: double-click the volume line to add a point, drag to adjust gain (default 100%, range 0–500%), and select a point then press Delete to remove it. A horizontal guide snaps to other point levels in the clip. Linear interpolation replaces the old fade handles; legacy fades become points on import. The curve multiplies clip volume for playback, Agent audio, mixing and export. Edits support undo; trimming and splitting preserve source-relative point positions.
- Select any non-subtitle Clip to set **Clip volume** from 0–500%; stored as `volume` (`1.0` = 100%) and applied to editor playback, Agent audio mixes, `clips_audio`, and Compose Video
...
- Linking a new generated video disables that Clip's older versions and mutes its detached audio in the trim editor; batch links enable only the newest entry. Save Latent context splice segments are rebuilt separately, retaining required neighboring footage and muting detached audio from replaced splice segments only. Files are not deleted, and entries can be manually re-enabled. Independent audio tracks on the main timeline are unchanged.
...
- Media-video **Mute / Unmute** applies immediately during timeline playback, is saved for export, and supports undo. Muted video Clips show a speaker-off indicator in the lower-right corner.
...
- The default action uses the **current workflow**: **Run and Preview** when the queue is idle keeps the modal open and switches to Video Preview. **Run (Add to Queue)** when tasks are running/pending only queues the Clip. Queue state is checked again on click.
- Connect KJNodes **Model Preview Override** in the MODEL path feeding the sampler for initial-noise and sampling previews. Set `preview_frames > 1` for animated previews when supported by the model/decoder. Models, seeds, resolution and sampling settings remain unchanged; this is a real generation/save run, not a separate 0.2MP preview. Without this node, only the final saved video is displayed.
- **Stop This Task** stops generation, not just playback, and targets only the verified task. Intermediate previews use local HTTP memory storage, not project assets; the saved video replaces them when ready. The **i** beside Video Preview explains setup.
- The previous imported workflow remains under **Separate preview workflow (optional)**. Its **Preview** button requires **Generate Timeline Preview** (`CAP_TimelinePreview`); the following low-resolution settings apply only to this separate mode.
