...
function respond(index, ok = true) {
    requests[index].resolve({ ok, json: async () => ({ config: { ...requests[index].body, has_key: !requests[index].body.clear_key }, error: 'Save failed' }) });
}
...
assert.match(root.innerHTML, /<cap-button data-bgm="clear_key" disabled>/);
const clear = ui.field('clear_key');
assert.equal(clear.disabled, false);
let clearing = clear.events.click();
assert.equal(clear.disabled, true);
await tick();
assert.equal(requests[4].body.clear_key, true);
assert.equal(requests[4].body.api_key, '');
respond(4, false);
await clearing;
assert.equal(clear.disabled, false, 'failed clear remains retryable');
assert.equal(ui.field('api_key').value, '****');
clearing = clear.events.click();
await tick();
respond(5);
await clearing;
assert.equal(clear.disabled, true, 'no saved key disables clear');
assert.equal(ui.field('api_key').value, '');
assert.equal(ui.field('key-status').textContent, 'bgm_key_missing');
const saving = ui.save();
await tick();
assert.equal(requests[6].body.clear_key, false, 'clear is a one-shot action');
respond(6);
await saving;
console.log('Clear key button: immediate action, failure retry and saved-state feedback passed');
