...
For Digital Human clips, Cap MiniMaxH3 encodes timeline audio into the output latent and sets its audio noise mask to zero. Pass the complete LATENT to the sampler; preserve the audio and its noise_mask when splitting, concatenating or refining. The audio output is the source track padded with context-prefix silence and to the model frame grid for saving. Trim both video and audio when removing context. MiniMax H3 Video Generator shares this preparation without encoding twice.
