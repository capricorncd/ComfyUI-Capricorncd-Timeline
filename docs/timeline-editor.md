...
- The right side provides AI Optimize and Video Preview tabs. Preview-workflow import, clearing, model placeholder, and load status are configured in Video Preview instead of the editor Settings dialog. Clicking the bottom Preview button switches to Video Preview before the preview workflow starts.
...
- `user/default/workflows/CapTimeLinePreview_v1.json` contains both the editable visual workflow and an embedded `output` API graph, so it can be imported directly from Video Preview.
