...
const expected = ['steps', 'strict_keyframes', 'attention', 'second_sampling', 'first_pass_megapixels',
    'upscaler_model', 'refine_sigmas', 'sampling_preview', 'preview_tiny_vae', 'generate_audio',
...
class Node {
    constructor() {
        ...
        this.widgets = oldWidgets.map(name => ({name, value: `default:${name}`}));
...
await extension.beforeRegisterNodeDef(Node, {name: 'CAP_H3VideoGenerator', input: {
    ...
    optional: Object.fromEntries(oldWidgets.slice(10).map(name => [name, {}])),
}});
const node = new Node();
node.onNodeCreated();
