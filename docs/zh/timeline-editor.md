...
本插件的核心编辑器，面向 ComfyUI 中的 **MV、短剧等长视频创作**。在多轨时间轴上将歌曲或剧情拆成多个 Clip，统一管理提示词、角色与场景参考素材，通过已连接的工作流逐段生成。检查并选择各镜头的生成结果，需要修改时只重做指定片段，最后将画面、音频和字幕剪辑合成为完整视频。

制作 MV 时，可围绕音乐编排镜头并调整时长；制作短剧时，可按剧情管理场景顺序和各 Clip 的参考素材。工程导入导出保留素材和工作流，便于继续编辑。长视频由多个片段组织合成，单段生成时长取决于模型和工作流。
...
所有接口配置保存在插件根目录 `config.local.yml`，可参考 `config.example.yml`。设置页读写该文件；未配置字段为空，已保存密钥显示 `****`。保持密钥不变或留空保存会保留原密钥。实际配置及临时文件由 Git 忽略，示例文件可以提交。配置按 `audio`、`voice`、`speech`、`agents` 分组，采用普通 YAML 字段。

## 本地音频服务

设置中的「音频服务」按通用、BGM/音乐、生成音效、音频降噪、人声分离、音色转换、文字转语音分为 Tabs。通用页设置服务器地址和 API Key；功能页的接口地址支持相对路径或完整 HTTP(S) URL，留空使用通用地址加默认接口路径。功能密钥留空继承通用密钥，已保存密钥显示 `****`，删除这四个字符并保存即可恢复继承。

第三方服务须兼容当前 Local AI Service 请求和异步任务协议。上传、音色列表、任务查询及下载使用对应接口的服务器和 `/v1/` 挂载前缀。当前不适配供应商特有的请求/响应格式。

编辑器不限制音频时长：按填写值或 Clip 实际时长发送，由服务器返回超限错误。文字转语音不传预计时长，插入模型实际生成的完整音频。

- **有音频的 Clip 右键 → 音色转换**：主时间轴、配音编辑和视频修剪均支持。可选择服务预设音色或项目中的参考录音，处理完整音频或 Clip 区间。结果插入对应音频轨道并静音原音频，不修改原文件。
- **字幕 Clip / 字幕轨道右键 → 文字转语音**：单 Clip 读取该条字幕；整轨按开始时间排序，每条字幕一行，只生成一条连续音频，插入主音频轨道的首条字幕起点，随后手动分割。不会自动填充字幕间隔。文本最多 2000 字，超限会提示修改而非截断。可选预设音色、语言和表达提示，或用项目中的 参考录音克隆音色；参考原文可选。
...
- 拖动 / 缩放片段；`Ctrl+点击` 多选
- 音频轨片段：双击波形音量线添加控制点，拖动调节音量（默认 100%，范围 0–500%），水平虚线可吸附本片段其他点的音量；选中点后按 Delete 删除。点间线性过渡，可直接制作淡入淡出；旧工程的淡入淡出载入时转为控制点。曲线叠加片段音量，应用于试听、Agent 音频、混音和导出；支持撤销，裁剪和分割保持源内位置。
- 选中任意非字幕 Clip，可将 **Clip 音量**设为 0–500%；工程保存为 `volume`（`1.0` = 100%），编辑器播放、发给 Agent 的音频混合、`clips_audio` 与「合成视频」都会应用
...
- 新关联生成视频时，自动禁用该 Clip 的旧视频版本，并静音修剪弹窗内已有的分离音频；批量关联默认只启用最新一条。Save Latent 的跨 Clip 衔接段单独重建，保留所需前后片段，只静音被替换衔接段的分离音频。旧版本及音频文件不会删除，可手动重新启用；主时间轴的独立音频轨不受影响。
- 关联生成视频列表递归读取 output，不限制为最新 400 条；默认按文件修改时间新到旧排列，可点击排序按钮切换为旧到新。“更早”显示 24 小时之前的文件；“自定义日期”按本地时间筛选单日，默认当天，可选择日期或使用左右按钮切换前一日、后一日。“选择文件”可从电脑目录选择视频并关联到当前 Clip：后台复用 output 内同名且内容一致的文件，否则原样保存到 `output/CapTimelineEditor/imports/`，重名时自动编号。浏览器不提供本地完整路径，因此选择已有文件时也需上传以核对内容。
- 可在播放头插入 Package / 素材
...
导入、恢复及导出 Schema 4 工程时，Clip 提示词和已有的全局前置/后置提示词按原文保留，不解析或重排 MiniMax H3 段落。`overall_soundscape` 可以留在各 Clip 中，不会被汇总到全局后置提示词。旧版字段兼容转换也不会把 Clip 内容搬到全局。

左侧最后的**完整提示词** Tab 只读显示用于生成的最终拼接文本：启用的全局前置 → 启用的素材描述 → Clip 提示词 → 启用的全局后置。随「使用提示词」选项更新，去除 `//` 注释行，不回写到 Clip 提示词。

富文本提示词框中，未选中文本时 **Ctrl+C** 复制光标所在整行；粘贴这种整行内容且没有选区时，插入到当前行下方。选中文字复制或从其他应用复制的普通文本，粘贴到光标位置；有选区时始终替换选区。按实际换行分行，不按视觉折行；Agent 提示词和 Prompt Skill 输入框同样支持 Ctrl+/ 切换 `//` 注释，生成时过滤注释行，Markdown 标题正常保留。
...
|--------|------|
| `Ctrl+B` | 禁用 / 启用选中片段 |
| `Ctrl+G` | 组合选中的 Clip（支持跨轨道、跨类型） |

多选后按 `Ctrl+B`：只要有可编辑的选中 Clip 处于启用状态，就全部禁用；全部禁用时则全部启用。音频 Clip 对应静音/取消静音，跳过锁定轨道，一次撤销恢复整批操作。
...
组合后点击任一成员会选中整组；拖动时保持各 Clip 的时长、相对间隔和素材偏移。组内存在锁定轨道时不允许整组移动。分组随工程保存，支持撤销/重做；复制粘贴产生独立的新组，不会联动原组。右键菜单也提供组合和取消组合。
...
素材管理面板标题栏最右侧的三点悬停菜单提供「清除未引用素材」。仅移除当前工程中的未引用素材记录，保留磁盘文件，可按 Ctrl+Z 撤销。禁用或锁定 Clip 的引用、分镜图片、角色参考音频、关联生成素材，以及视频裁剪、镜头切分和图片裁剪的来源素材均保留。
素材卡片仅标记「已使用」和「被引用」，分别使用主题强调色和琥珀色；未引用素材不显示状态标签。
